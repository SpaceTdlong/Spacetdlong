...
@interface HuGeDatePickerView ()<UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation HuGeDatePickerView
...
- (void)configData {
    ...
    NSDate *afterDate = [NSDate dateWithTimeInterval:1 * 60 * 60 sinceDate:date];
    NSDateFormatter* dateFormatter = [[NSDateFormatter alloc]init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm"];
    
    self.date = [dateFormatter stringFromDate:afterDate];
}
...
- (void)saveBtnClick {
    ...
    self.selectStr = [NSString stringWithFormat:@"%ld-%@-%@  %@:%@:36", [self.year integerValue], month, day, hour, minute];
